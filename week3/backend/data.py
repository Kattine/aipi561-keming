...
_dq_logger = _logging.getLogger(__name__)

# Path to the new upstream data file with potential quality issues
_CORRUPTED_DATA_PATH = _ROOT / "week3" / "data" / "demand_enriched_corrupted.parquet"
_QUALITY_CUTOFF = pd.Timestamp("2026-01-16")


def check_and_log_data_quality():
    """
    Run data quality validation on the corrupted parquet at startup.

    Design principles
    ─────────────────
    1. NEVER raises — the API must start regardless of data quality.
       All issues are surfaced through WARNING/ERROR log entries so
       operators see them immediately in their log aggregator.
    2. Loads a baseline (pre-cutoff) window from the same file for
       comparison-based checks (outliers, categorical rate anomalies).
    3. The entire function is wrapped in a broad try/except so even
       import failures or I/O errors cannot crash the API.
    """
    try:
        if not _CORRUPTED_DATA_PATH.exists():
            _dq_logger.warning(
                f"[DataQuality] Corrupted data file not found at "
                f"{_CORRUPTED_DATA_PATH}. Skipping quality check."
            )
            return

        _dq_logger.info(f"[DataQuality] Running checks on {_CORRUPTED_DATA_PATH} ...")
        df        = pd.read_parquet(_CORRUPTED_DATA_PATH)
        baseline  = df[df["time_bucket"] < _QUALITY_CUTOFF].copy()
        corrupted = df[df["time_bucket"] >= _QUALITY_CUTOFF].copy()

        _dq_logger.info(
            f"[DataQuality] Baseline: {len(baseline):,} rows | "
            f"New window: {len(corrupted):,} rows"
        )
        ...
        result    = validator.validate(corrupted, baseline_df=baseline)

        if result["is_valid"]:
            _dq_logger.info("[DataQuality] ✅ All checks passed — no issues found.")
        else:
            _dq_logger.warning(
                f"[DataQuality] ⚠️  {result['num_issues']} issue(s) detected:"
            )
            for issue in result["issues"]:
                _dq_logger.warning(
                    f"[DataQuality]   [{issue['severity'].upper():8s}] "
                    f"{issue['type']:30s}: {issue['description']}"
                )

    except ImportError as exc:
        _dq_logger.error(
            f"[DataQuality] Could not import DataQualityValidator "
            f"(is week3/validation/ on PYTHONPATH?): {exc}"
        )
    except Exception as exc:
        # Catch-all: log and continue. API must not crash here.
        _dq_logger.error(
            f"[DataQuality] Check failed unexpectedly: {exc}",
            exc_info=True,
        )


# ── Run data quality check at startup (Week 3) ───────────────────────────────
